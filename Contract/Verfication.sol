...
contract Verification {
    constructor() { owner = msg.sender; } //Fungsi ini dipanggil saat kontrak pertama kali dibuat. Ini mengatur pemilik kontrak menjadi pengirim pesan saat ini.
...
//---------------------------------------------------------------------------------------------------------//
    ...
    function add_Exporter(address _add,string calldata _info) external //Fungsi ini memungkinkan pemilik untuk menambahkan Eksportir baru ke kontrak
    onlyOwner(){ 
    ...
    function delete_Exporter(address _add) external  //Fungsi ini memungkinkan pemilik untuk menghapus Eksportir dari kontrak
    onlyOwner
    {
        assert(Exporters[_add].blockNumber!=0);
    ...
    function alter_Exporter(address _add,string calldata    _newInfo) public //Fungsi ini memungkinkan pemilik untuk mengubah informasi tentang Eksportir.
    onlyOwner()
     { 
          assert(Exporters[_add].blockNumber!=0);
             Exporters[_add].info=_newInfo; }

    function changeOwner(address _newOwner) public  //Fungsi ini memungkinkan pemilik untuk mengubah kepemilikan kontrak
        onlyOwner  validAddress(_newOwner)   {  owner = _newOwner; }

        event addHash(address indexed _exporter,string _ipfsHash);
    function addDocHash (bytes32  hash,string calldata _ipfs) public //Fungsi ini memungkinkan Eksportir untuk menambahkan hash dokumen ke kontrak
      canAddHash
      {
    ...
    function findDocHash (bytes32 _hash) //Fungsi ini memungkinkan pengguna untuk mencari hash dokumen dalam kontrak dan mengembalikan informasi terkait.
    ...
    function deleteHash (bytes32 _hash) public //Fungsi ini memungkinkan Eksportir untuk menghapus hash dokumen dari kontrak.
    ...
    function  getExporterInfo(address _add) external view returns(string memory){ //Fungsi ini memungkinkan pengguna untuk mendapatkan informasi tentang Eksportir
